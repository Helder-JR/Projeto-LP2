...
public class Civil implements Funcao {
    ...
    public String toString(String nome, String dni, String estado, String partido, String interesses) {
        if ("".equals(partido.trim()) && !"".equals(interesses.trim())) {
            return String.format("%s - %s (%s) - Interesses: %s", nome, dni, estado, interesses);
        }
        if ("".equals(interesses.trim()) && !"".equals(partido.trim())) {
            return String.format("%s - %s (%s) - %s", nome, dni, estado, partido);
        }
        if ("".equals(interesses.trim()) && "".equals(partido.trim())) {
            return String.format("%s - %s (%s)", nome, dni, estado);
        }
        return String.format("%s - %s (%s) - %s - Interesses: %s", nome, dni, estado, partido, interesses);
    }
}
